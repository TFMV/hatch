...
RUN openssl s_client -showcerts -connect repository.bog.ge:443 </dev/null \
  | sed -n -e '/-.BEGIN/,/-.END/ p' > /usr/local/share/ca-certificates/bog.crt \
  && update-ca-certificates

RUN openssl s_client -showcerts -connect go.dev:443 </dev/null \
  | sed -n -e '/-.BEGIN/,/-.END/ p' > /usr/local/share/ca-certificates/go.crt \
  && update-ca-certificates
...
RUN go install golang.org/x/tools/gopls@latest golang.org/x/tools/gopls@latest
